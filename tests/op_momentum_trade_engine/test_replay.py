...
class TestBarReplayDriverTimeline:
    ...
    def test_build_timeline_default_exit_excludes_1555_bar(self):
        bars = {
            "NVDA": [
                _bar("NVDA", 15, 50),
                _bar("NVDA", 15, 55),  # at cutoff — excluded (strict <) to match BT
                _bar("NVDA", 16, 0),   # after market close — also excluded
            ],
        }
        driver = BarReplayDriver(tickers=["NVDA"], replay_date=date(2026, 3, 17), signal_engine=MagicMock())
        timeline = driver._build_timeline(bars)
    ...
    def test_build_timeline_full_day_exit_time_excludes_1555_bar(self):
        bars = {
            "NVDA": [
                _bar("NVDA", 14, 55),
                _bar("NVDA", 15, 0),
    ...
    def test_build_timeline_excludes_bar_at_and_after_exit_time(self):
        bars = {
            "NVDA": [
                _bar("NVDA", 15, 50),
                _bar("NVDA", 15, 55),  # at exit_time — excluded by strict <
                _bar("NVDA", 16, 0),   # after exit_time — also excluded
            ],
        }
        driver = BarReplayDriver(
            tickers=["NVDA"],
