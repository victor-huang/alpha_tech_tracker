...
| `research/experiments/OR_WINRATE_STRATEGY_STUDY.md` | 8-year stock study of OR-direction + trailing-win-rate entries: best configs, cost sensitivity, 7 robust findings, negative results (put/call untestable, MA confirm hurts, adaptive trailing loses), and 5 harness defects found and fixed |
...
| `analysis_scripts/ticker_stats_report.py` | "For each ticker, what happens after the OR closes above/below its midpoint?" — splits sessions into bull/bear by the opening-window close, then reports follow-through rate, avg/best max favourable excursion, and 15:50 win rate with avg/best/worst return (signed with the setup). Also prints monthly option OI call/put ratios at several strike bands, opening-window volume vs its own 20-session baseline, and 20-session range%/drift% stats with an optional histogram PDF. Defaults to `--feed sip`, clamped to the last completed session. |
| `analysis_scripts/or_winrate_pc_backtest.py` | "Does an OR-direction entry on high-win-rate tickers make money as stock?" — selects top-N by trailing 10-session 15:50 win rate, enters long/short on the OR-midpoint break at 09:45, exits on a percent hard stop, an MA trailing stop on 5-min closes (optionally switched by daily-MA-stack regime), or a configurable time exit. Gates: prior-close benchmark regime, rolling-ADR distance from the daily MA20, OR volume vs its own 20-session baseline, OR range. `--cost-bps` charges round-trip friction. See `research/experiments/OR_WINRATE_STRATEGY_STUDY.md` before changing defaults. |
| `analysis_scripts/dt_watch_list_log.py` | **Daily watch-list driver.** Writes `dt_stock_watch_list_log/<as_of>/` containing ticker_stats.txt, watchlist.md (gate state + mechanical long/short candidates), option_open_interest.json and a range% histogram. Follow `dt_stock_watch_list_log/SKILL.md` — the ranking is mechanical and needs a review pass. Pass `--skip-options` with a past `--end`, since Alpaca serves open interest live-only. |
