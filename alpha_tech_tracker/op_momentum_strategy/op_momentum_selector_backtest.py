...
from alpha_tech_tracker.op_momentum_strategy.backtest.constants import (  # noqa: F401
    DOUBLEDOWN_START_MIN,
    INITIAL_CAPITAL,
    MIN_WINDOW_CAPITAL,
    REGIME_ADAPTIVE_CONFIGS,
    REGIME_MA_STRONG_SCORE,
    REGIME_VIX_HI,
    REGIME_VIX_LO,
    _EOD_DISPLAY_TIME,
)
# _parse_weights is imported from this module by ~19 analysis scripts — the
# re-export below keeps those callers working.
from alpha_tech_tracker.op_momentum_strategy.backtest.weights import (  # noqa: F401
    _effective_weights,
    _parse_weights,
    _weights_label,
)
from alpha_tech_tracker.op_momentum_strategy.backtest.args import (  # noqa: F401
    _parse_args,
    arg_error,
    build_parser,
)
from alpha_tech_tracker.op_momentum_strategy.backtest.reporting import (  # noqa: F401
    _bnh_period_groups,
    _capital_stats_from_trades,
    _period_capital_groups,
    _print_bnh_period_table,
    _print_capital_stats_block,
    _print_daily_table,
    _print_day_summary,
    _print_opportunity_pool_block,
...
def _compute_bear_ctp_dates(
    ...
    Default: dates where prior-day QQQ regime factor == 1.0 (full bear).

    qqq_regime_bear_ctp_ma_cross: also requires prior-day MA20 < MA50.
    qqq_regime_bear_ctp_below_ma: use price-relative condition instead of
...
def run_selector_backtest(
    ...
    if qqq_regime_bear_ctp is not None and not _qqq_df.empty:
        ...
        print(
            f"  Bear CTP dates: {len(_bear_ctp_dates)} days"
            f"  bear_ctp={qqq_regime_bear_ctp:.2f}"
            + (f"  {_ctp_mode}" if _ctp_mode else "")
        )
    ...
    qqq_or_by_date: dict = {}
    if qqq_or_weight != 0.0 and not _qqq_df.empty:
    ...
    if qqq_regime_weight != 0.0 and not _qqq_df.empty:
        _rbd = {d_: g for d_, g in _qqq_df.groupby(_qqq_df.index.date)}
        _rdates = sorted(_rbd.keys())
        _rcloses = [float(_rbd[d_]["Close"].iloc[-1]) for d_ in _rdates]
        ...
        for d_ in trading_days:
            ...
            if _ri is None or _ri == 0:
                continue
            _pi = _ri - 1  # prior trading day index
            _rc = _rcloses[_pi]
            _rm20 = float(_rma20.iloc[_pi])
...
if __name__ == "__main__":
    ...
    if args.window_ma_switch:
        ...
        for entry in args.window_ma_switch:
            if len(entry) < 2:
                arg_error(f"--window-ma-switch requires LABEL MODE [PERIOD], got: {entry}")
            wlabel, mode = entry[0], entry[1]
            if mode not in _valid_modes:
                arg_error(f"--window-ma-switch mode must be one of {_valid_modes}, got: {mode}")
            if wlabel not in _win_by_label:
                arg_error(f"--window-ma-switch label '{wlabel}' not found in defined windows")
    ...
    if args.qqq_regime_weight != 0.0:
        _regime_mode = []
        if args.qqq_regime_full_only:
