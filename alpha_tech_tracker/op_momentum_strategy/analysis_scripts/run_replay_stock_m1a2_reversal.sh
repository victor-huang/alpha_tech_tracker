#!/usr/bin/env bash
# run_replay_stock_m1a2_reversal.sh
# Replay trades for 2026 (up to end date) with custom 2-window config.
# Config: M1 09:30/1 + A2 10:15/2, reversal, stop-pct 0.6, feed sip
#
# Usage:
#   ./run_replay_stock_m1a2_reversal.sh --year 2026
#   ./run_replay_stock_m1a2_reversal.sh --year 2026 --end-date 2026-05-15
#   ./run_replay_stock_m1a2_reversal.sh --year 2026 --summary
#   ./run_replay_stock_m1a2_reversal.sh --year 2026 --force
...
if [ -z "$YEAR" ]; then
  echo "Usage: $0 --year YYYY [--end-date YYYY-MM-DD] [--trade-type stock|options] [--summary] [--force]"
  exit 1
fi

LOG_DIR="$BASE_LOG_DIR/replay_${YEAR}_${TRADE_TYPE}_m1a2_reversal"
...
if end_date_arg:
    end = date.fromisoformat(end_date_arg)
if end > today:
    end = today
...
print()
print(f"=== {year} Stock Replay — M1 09:30/1 + A2 10:15/2 ===")
print(f"    reversal | stop-pct 0.6 | feed sip | top2 | $10k")
print()
...
echo ""
echo "=== $YEAR $TRADE_TYPE replay (M1 09:30/1 + A2 10:15/2, reversal, stop-pct 0.6) ==="
