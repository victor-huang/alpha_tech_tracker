...
def save_ticker_min_agg_to_json(agg_data):
    ...
    selected_agg_data = {
        k: v for k, v in agg_data.dict().items() if k in selected_agg_attributes
    }
...
class DataAggregator(object):
    ...
    def __init__(self):
        self.key_id = os.environ.get("ALPACA_API_KEY")
    ...
    def add(self, data):
        selected_data = {
            k: v for k, v in data.dict().items() if k in self.selected_agg_attributes
        }

        new_series = pd.Series(selected_data)
        new_series.reset_index(0, drop=True)
        new_series["timestamp"] = new_series["timestamp"].astimezone(
        ...
        if len(self.raw_data_df) >= 5 and (
            latest_timestamp.minute % 10 == 4 or latest_timestamp.minute % 10 == 9
        ):
