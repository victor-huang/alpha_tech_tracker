...
class SimpleStrategy(Strategy):
    ...
    def simulate(self, *, start='2019-03-13', end='2019-05-09', use_saved_data=False, stream_data=False, market_data_file_date_rage=[]):
        ...
        pp.pprint(f"number_of_profit_positions: {pln_info['number_of_profit_positions']}")
    ...
    def check_open_position_condition(self):
        ...
        if (self.risk_reward_ratio(current_price, waves=waves) > self.buy_trigger_risk_reward_ratio
            ...
            is_likely_at_end_of_a_up_wave = (
                    (last_3_waves_directions == ['up', 'up' , 'up']) and (waves[-1].length() > waves_stats['average_wave_length'] * 1.3)
            ...
            if ((waves_stats['up_waves_ratio'] >= self.buy_trigger_up_waves_ratio
                    ...
                    or (self.has_strong_buy_after_sell_off(waves_stats) and not is_likely_at_end_of_a_up_wave)
                ...
                print(f"-----end? {is_likely_at_end_of_a_up_wave}, {last_3_waves_directions[-1]}, last wave length: {waves[-1].length()}, avg wave length x2 {waves_stats['average_wave_length'] * 2} ")
    ...
    def is_maximum_loss_reached(self, position_id):
        ...
        print(f'******current loss: {((position.strike_price + position.open_price) - Decimal(self.current_time_period_price())) * 100} , open price: {position.open_price}, current price: {self.current_time_period_price()}')
        # 1000
        # s 900  990
        # ca 120 - 900
        # self.target_option_strike_price_delta
        if ((position.strike_price + position.open_price) - Decimal(self.current_time_period_price())) * 100 >= self.maximum_position_loss:
            return True
    ...
    def order_event_handler(self, order):
        if self.active_order_to_position_map[order.id] == None:
            ...
            new_position = self.portfolio.add_position(symbol=order.symbol, open_price=Decimal(round(order.executed_price, 2)), type=self.asset_type, quantity=order.quantity, open_at=open_at, open_order_id=order.id, osi_key=order.osi_key, strike_price=order.strike_price)
