---
description: 'Run full-year trade engine replay (stock, M1 09:30/3 + A1 10:00/3 + A2 11:45/2 + A3 13:15/1 + A4 15:15/1, parallel of 20) and report weekly + monthly P&L'
---

# Full-Year Stock Replay (5-window)
...
Set:
- `SCRIPT` = `/Users/victorhuang/work/alpha_tech_tracker/run_replay_stock_5win.sh`
- `LOG_DIR` = `/Users/victorhuang/work/alpha_tech_tracker/logs/replay_${YEAR}_stock_5win`
...
The script runs 20 days in parallel per batch. It will:
1. Skip days that already have a log (unless `--force`)
2. Print `OK` / `ERR` per day as each batch completes
3. Print the full weekly + monthly + total P&L summary when done
...
3. **Monthly summary** — each month's total P&L and % return
4. **Total** — total capital P&L and % return on $10k
